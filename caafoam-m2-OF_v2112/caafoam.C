...
//
	...
	//momentum viscous flux
        ...
        volVectorField momVisFlux = fvc::laplacian(muEff, U) + fvc::div(tauMC);
	//energy viscous flux
        sigmaU = ( (( (2.0/3.0)*muEff*fvc::div(U))*I + 2.0*muEff*symm(fvc::grad(U))) & U );
        volScalarField heatFlux = fvc::laplacian(k, T);
        volScalarField visWork  =  fvc::div(sigmaU);
